...
export class CommentService extends BaseService<CommentDocument> {
  ...
  @InjectModel('commentFavorites') private commentFavoriteModel: Model<CommentFavoriteDocument>;
  constructor(@InjectModel('comment') public commentModel: Model<CommentDocument>) {
    super(commentModel)
  }
  ...
  async getListForComments(query: FilterQuery<CommentDocument>, uid: string, lean: Boolean, projection?: string) {
    ...
    const favorites: CommentFavoriteDocument[] = await this.commentFavoriteModel.find({ createdBy: uid, articleId: query.articleId });
    ...
    const users = await this.userModel.find({ uid: { $in: uids } }, 'uid realName');
  ...
  private _cascaderForComments(comments: CommentDocument[], result: any[], userKeyByUid: any) {
